...
export function useGenerate() {
  ...
  async function generate(params: SynthParams) { setState('generating'); setResult(null); setError(''); setProgress(params.text.length > 1200 ? 0 : null); try { const next = params.text.length > 1200 ? await api.synthStream(params, setProgress) : await api.synth(params); setResult(next); setState('success') } catch (err) { setError(mapErrorToLimitKind(err) || 'generic'); setState('error') } }
