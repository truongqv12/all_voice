...
export function MyClonesList({ clones, onDeleted }: { clones: VoiceClone[]; onDeleted(id: string): void }) {
  ...
  async function remove(clone: VoiceClone) { if (!window.confirm(t('clone.deleteConfirm', { name: clone.name }))) return; await api.deleteClone(clone.id); onDeleted(clone.id) }
