...
export function VoiceGrid({ voices, loading, error, onRetry, onReset, onSelected }: Props) {
  ...
  if (loading) return <div className="grid grid-cols-1 gap-3">{[1, 2, 3, 4].map(index => <Skeleton key={index} className="h-48" />)}</div>
  ...
  const cards = (items: Voice[]) => items.map(voice => <VoiceCard key={voice.id} voice={voice} selected={voice.id === selectedVoice?.id} active={preview.activeId === voice.id} loading={preview.loadingId === voice.id} previewUnavailable={preview.errorId === voice.id} onToggle={() => void preview.toggle(voice)} onSelect={() => { selectVoice(voice); onSelected?.() }} />)
  return <div className="space-y-3">{yours.length > 0 && <section><h3 className="mb-2 text-sm font-semibold text-[var(--color-primary)]">{t('voice.yours')}</h3><div className="grid grid-cols-1 gap-3">{cards(yours)}</div></section>}<div className="grid grid-cols-1 gap-3">{cards(catalog)}</div></div>
}
